...
using static System.Math;
    ...
    public class Graph
    {
        ...
        public void printSchemaAsGEXF(string path="./DotFiles/outputGraph.gexf")
        {
        ...
        public void printSchemaAsDOT(string path="./DotFiles/outputGraph.dot")
        {
        ...
        public void printCompactSchemaAsDOT(string path="./DotFiles/outputCompactGraph.dot")
        {
            using (var dot = new System.IO.StreamWriter(path))
            {
                dot.WriteLine("digraph G");
   ...
   public void addLayers()
        {
            ...
            while(q.Count>0)
            {
                ...
                {
                    MFALogger.DualWriter.WriteLine("All neighbors of {"+element.ToString()+"} visited.");
                    //item=null;
                ...
                {
                    this.quantumErrors++;
                    MFALogger.DualWriter.WriteLine("Quantum run failed, index out of range. Retrying. ");
                }
                else if(nodesLayer[(int)itemIndex]!=-1)
                {
                    this.quantumErrors++;
                    MFALogger.DualWriter.WriteLine("Quantum run failed, selected value hasn't infinite layer value. Retrying.");
                }
                else
                {
                    MFALogger.DualWriter.WriteLine("Item found:"+reachableNeighbors[(int)itemIndex].ToString());
                    ...
                    if(this.sinkNode.Layer==-1)// -1 = infinite layer value
                    {
                        ...
                        MFALogger.DualWriter.WriteLine("Item updated to "+reachableNeighbors[(int)itemIndex].ToString()+" and enqueued.");
                    }
                    else
                    {
                        MFALogger.DualWriter.WriteLine("Item updated to "+reachableNeighbors[(int)itemIndex].ToString()+".");
                        MFALogger.DualWriter.WriteLine("Item was the sink node. Layering task has finished.\n"+
        ...
        {//Looks for a path from source to sink node
                ...
                //Quantum part
                    ...
                    do
                    {
                        ...
                        else if(nodesLayer[(int)itemIndex]!=layerNumber)
                        {
                            ...
                            MFALogger.DualWriter.WriteLine($"Quantum run failed, selected value hasn't {layerNumber} layer value. Retrying.");
                            this.quantumErrors++;
                        }
                        else
                        {
                            MFALogger.DualWriter.WriteLine("Item Index:"+itemIndex.ToString());
...
private long findInfiniteLayerNeighbors(QuantumSimulator sim,List<Node> neighbors,List<long> associatedLayers)
    ...
    List<long> associatedLayers,long layerNumber)
        {
            ...
            //MFALogger.DualWriter.WriteLine($"{markedElements.Count} elements marked for Grover Search.");
            ...
            MFALogger.DualWriter.Write("Elements to be found are: [ ");
            foreach (var item in markedElements)
            {
                ...
                MFALogger.DualWriter.Write(item.ToString()+", ");        
            }
            MFALogger.DualWriter.WriteLine("] in a dataset of size "+neighbors.Count.ToString());
            MFALogger.DualWriter.WriteLine("Actual size of the dataset: "+Math.Pow(2,nDatabaseQubits).ToString());        
            int nIterations;
            if(r.Next(0,2)==0)
            ...
            if (nMarkedElements==0)
                return -1;
            MFALogger.DualWriter.WriteLine("Oracle + Diffusion iterations: "+nIterations.ToString());    
            var task = MFA.ApplyGroverSearch.Run(sim, new QArray<long>(markedElements), nIterations, nDatabaseQubits);
            var data = task.Result;
            MFALogger.DualWriter.WriteLine("Outcome is "+task.Result);
             if(data.Item1==0)
             {MFALogger.DualWriter.WriteLine("Grover's algorithm failed in finding a result.");}
    ...
    public class Edge
    {
        ...
        public Edge(Node Source,Node Destination, long Capacity)
        {
            ...
            MFALogger.DualWriter.WriteLine("Created edge {"+this.ToString()+"}");
        ...
        public long getMaxFlow()
        {
            ...
            //MFALogger.DualWriter.WriteLine($"getMaxFlow: {this.ToString()}  isFlowReversed: {isFlowReversed()}");
            if(isFlowReversed())
            {
                return Stream;
            }
...
public long applyMaxFlow()
{
    ...
    {
        if(Edges.Count==0)
        {MFALogger.DualWriter.WriteLine("Calculating max flow which is 0");
            return 0;
        }
        else
        {
            MFALogger.DualWriter.WriteLine("Calculating max flow");
            ...
            MFALogger.DualWriter.WriteLine($"max flow={maxflow}");
            return maxflow;
        } 
    }
