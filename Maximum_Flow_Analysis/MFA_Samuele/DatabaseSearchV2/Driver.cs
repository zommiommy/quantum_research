...
namespace Quantum.DatabaseSearch
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            QArray<int> array = new QArray<int>() { 1, 5, 8, 9, 13 };
            int target = 8;
            performListSearch(sim, repeats, groverIterations, array, target);
            Pause();
        ...
        static void performListSearch(
            ...
            int target)
        {
            long listSize = array.Length;
            int listBits = (int)Math.Ceiling(Math.Log(listSize, 2));
            Console.WriteLine($"List length: {array.Length}. Bits needed: {listBits}.");
            ...
            // We now execute the quantum search and verify that the success 
            // probability matches the theoretical prediction. 
            double classicalSuccessProbability = (double)1 / listSize;
            double quantumSuccessProbability = Math.Pow(Math.Sin((2.0 * (double)groverIterations + 1.0) * Math.Asin(Math.Sqrt(1) / Math.Sqrt(listSize))), 2.0);
            int successCount = 0;

            Console.Write(
                $"\n\nQuantum search for marked element in database.\n" +
                $"  List size: {listSize}.\n" +
                $"  List elements: {string.Join(",", array.Select(x => x.ToString()).ToArray())}\n" +
