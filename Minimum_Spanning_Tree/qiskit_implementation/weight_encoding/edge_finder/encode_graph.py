...
def encode_graph(circuit : QuantumCircuit, graph : List[Tuple[int, int, float]], start : QuantumRegister, end : QuantumRegister, weight : QuantumRegister, target : QuantumRegister, ancillas : QuantumRegister) -> QuantumCircuit:
    """Encode all the edges of the graph."""
    
    for start_val, end_val, weight_val in graph:
        circuit = encode_edge(circuit, start, start_val, end, end_val, weight, weight_val, target, ancillas)
