...
class SelectMultipleVertices(EdgeFinder):
    ...
    def setup_oracle(self):
        ...
        # Select the ending vertex to be == 4
        self.oracle = encode_value(self.oracle,  self.end, 4, self.ancillas)

        controlled = [q for register in [self.end, [self.flags[0]]] for q in register]
        self.oracle.cnx(controlled,self.flags[1],self.ancillas)

        self.oracle = encode_value_inverse(self.oracle,  self.end, 4, self.ancillas)
